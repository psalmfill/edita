...
@section('content')
    <div class="auth-form-light text-left py-5 px-4 px-sm-5">
        ...
        <form class="pt-3" method="post" action={{ route('do.staff_login') }}>
            @csrf
            <div class="form-group">
                <input type="text" class="form-control form-control-lg {{ $errors->has('email') ? 'border-danger' : '' }}"
                    id="email" name='email' value="{{ old('email') }}" placeholder="Email" required>
                @if ($errors->has('email'))
                    <div class="pt-1 text-danger">{{ $errors->first('email') }}</div>
                @endif
            </div>
            <div class="form-group">
                <input type="password"
